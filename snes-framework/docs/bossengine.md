...
the Bowser-arena path, which calls `CODE_00B4D3 / CODE_00BB90`
instead of the regular palette/VRAM init -- a different special-case
boss-arena loader.
...
3. **`$1015` — Kamek spell handshake** — **RESOLVED.** Not a frame counter. `$1015` is a 16-bit **bidirectional handshake signal** between boss state machines and the CutsceneKamek sprite (`$048`, dispatched from `Bank0C:11905` via `DATA_cutscene_kamek_state_ptr`). 4-state protocol:
...
5. **`$107E / $107C` boss-hit pair** — **RESOLVED.** Hypothesis confirmed, with a refinement. `$107C` is the **cumulative** ground-pound hit counter (advances 0 → 2 → 4 → 6 = defeat). `$107E` is a **one-shot latch snapshot** of `$107C` taken at the moment Hookbill becomes grounded and vulnerable — not a static "expected hits" threshold.
...
7. **Naval Piranha bud sub-state `$0E` (`bud_state_anchored`) world-space anchoring** — **RESOLVED.** Not "snapshotting world position" — `$0E` is a **physics-cancelling freeze**. The mechanism uses the engine's standard per-frame sprite-velocity-delta cache. The earlier framing ("snapshots world-space position relative to the camera") was wrong.
