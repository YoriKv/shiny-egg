...
// real V1.0 cart: each animation type must overwrite exactly the CGRAM color
...
/** Set of CGRAM COLOR indices the overlay writes, found via a two-sentinel
...
// Single-row types: written colors == the asm target row exactly.
{
  // $01 anim_pal_01 → X=$86 (color 67), 26 bytes = colors 67..79.
  assert(eqSet(writtenColors(header(0x01)), range(67, 79)), `type 0x01 writes colors 67-79 (got ${fmt(writtenColors(header(0x01)))})`);
  // $02 dir-aware base → X=$0A (color 5), 6 bytes = colors 5..7.
  assert(eqSet(writtenColors(header(0x02)), range(5, 7)), `type 0x02 writes colors 5-7 (got ${fmt(writtenColors(header(0x02)))})`);
  // $03 → X=$E0 (color 112), 32 bytes = colors 112..127.
  assert(eqSet(writtenColors(header(0x03)), range(112, 127)), `type 0x03 writes colors 112-127 (got ${fmt(writtenColors(header(0x03)))})`);
  // $09 → CGRAM colors 1 and 9 (one shared word).
  assert(eqSet(writtenColors(header(0x09)), [1, 9]), `type 0x09 writes colors 1,9 (got ${fmt(writtenColors(header(0x09)))})`);
  // $0F → X=$0A (color 5), 6 bytes = colors 5..7.
  assert(eqSet(writtenColors(header(0x0f)), range(5, 7)), `type 0x0f writes colors 5-7 (got ${fmt(writtenColors(header(0x0f)))})`);
}

// Header-conditional: $0D selects its source by BG3 palette bit 0 but writes the
// SAME row (colors 1..3) either way; $05 c644 prefix only fires for BG1&7==0.
{
  assert(eqSet(writtenColors(header(0x0d, { bg3Palette: 0 })), range(1, 3)), 'type 0x0d (BG3pal even) writes colors 1-3');
  assert(eqSet(writtenColors(header(0x0d, { bg3Palette: 1 })), range(1, 3)), 'type 0x0d (BG3pal odd) writes colors 1-3');
  // $05: BG1&7==0 → c644 adds colors 2-7 + 67-79 on top of c5c1's 113-120.
